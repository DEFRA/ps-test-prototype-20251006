...
> [!WARNING]
> The `ps-test-prototype-20251006` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `ps-test-prototype-20251006`. To turn it
...
```bash
docker build --target development --no-cache --tag ps-test-prototype-20251006:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 ps-test-prototype-20251006:development
```
...
```bash
docker build --no-cache --tag ps-test-prototype-20251006 .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 ps-test-prototype-20251006
```
...
```bash
docker run -it --entrypoint /bin/ash ps-test-prototype-20251006
```
